...
Daily aggregator of Extra aircraft classified listings from
...
- `scraper/barnstormers.py` searches Barnstormers.com's
  [Aerobatic - Extra category](https://www.barnstormers.com/category-15856-Aerobatic--Extra.html)
...
- Only whole-aircraft-for-sale listings are published. Each ad's title must match a
  ...
  **`YEAR Extra MODEL`** form when the ad states a model year (e.g. `2016 Extra 330LX`),
  or just **`Extra MODEL`** when it doesn't - a missing year isn't disqualifying, since
  plenty of genuine ads simply don't state one in the title - regardless of how the
...
- `main.py` runs the scraper, de-duplicates results, and renders them into
  `docs/index.html` titled **"Other Extra Ads on the Web"**, with one row per
...
4. GitHub will publish the page at `https://taildraggers.github.io/extra/`
...
```html
<iframe
  src="https://taildraggers.github.io/extra/"
  title="Other Extra Ads on the Web"
